    ...
    */
    ...
    if ($message != null) {
        ...
        if (!$sloodle->module->add_message($message)) {
            ...
            sloodle_add_to_log($sloodle->course->get_course_id(), 'mod_log', 'mod/chat-1.0/linker.php', array(), 'chat: Failed to add chat message to chatroom');
        }
        else {
            //add_to_log($sloodle->course->get_course_id(), 'sloodle', 'add message', '', 'Added chat message to chatroom', $sloodle->request->get_module_id());
            sloodle_add_to_log($sloodle->course->get_course_id(), 'mod_log', 'mod/chat-1.0/linker.php', array(), 'chat: Added chat message to chatroom');
        }
    }
